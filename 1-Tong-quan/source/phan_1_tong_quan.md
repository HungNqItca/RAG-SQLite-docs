...
- `type_tab_hint`: với tabular, gợi ý sub-type cụ thể (`PHI-DCTC`, `PHI-TOCHUC`, `PHI-CANHAN`, `LAI_SUAT`…) để Phase 2 có thể filter sớm.
...
| R_DOC_REF | Khớp mã văn bản pháp lý VN dạng `số/năm/loại-cơ_quan` (vd `15/2024/TT-NHNN`, `13/2023/NĐ-CP`). Đặt **sau** R2–R6 (có chủ đích) để các rule tabular vẫn thắng khi câu hỏi có cả mã văn bản lẫn từ khóa phí/lãi. | "15/2024/TT-NHNN quy định gì?" | **legal** | 0.92 |
...
> **Vì sao `R_DOC_REF` có confidence 0.92 và đặt sau R2–R6?** Đây là rule nhận diện mã văn bản pháp lý đầy đủ (`15/2024/TT-NHNN`). Nó được đặt **sau** các rule tabular (R2–R6) để khi một câu vừa có mã văn bản vừa có từ khóa phí/lãi thì nhánh tabular vẫn được ưu tiên. Confidence 0.92 (≥ 0.90) được chọn có chủ đích để kích hoạt *confidence guard* trong `_prepare_unified()` - đảm bảo câu nêu đích danh mã văn bản được giữ đúng nhánh legal trong chế độ Unified dispatch.

Ngoài `content_type`, classifier còn trả về `type_tab_hint` - gợi ý sub-type để Phase 2 filter theo loại biểu phí cụ thể khi classifier đủ tự tin (`confidence ≥ 0.80`). Các giá trị hint thực tế: `PHI-DCTC` (phí cho định chế tài chính), `PHI-TOCHUC` (phí cho tổ chức), `PHI-CANHAN` (phí cho cá nhân), và `LAI_SUAT` (lãi suất). Khi phát hiện ngữ cảnh phí, hint được chọn theo **thứ tự ưu tiên DCTC > TOCHUC > CANHAN**; nếu là câu hỏi phí chung chung không rõ đối tượng khách hàng, hint trả về `None` (tìm trên tất cả loại PHI).
